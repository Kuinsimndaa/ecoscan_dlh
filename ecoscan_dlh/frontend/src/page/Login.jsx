...
const Login = () => {
  ...
  const defaultEmailByTab = activeTab === 'admin' ? 'admin@tegal.go.id' : 'petugas@ecoscan.com';
  const emailHelpText = activeTab === 'admin' ? 'Gunakan email admin@tegal.go.id untuk Admin' : 'Gunakan email petugas@ecoscan.com untuk Petugas Scan';

  const handleTabChange = (tab) => {
    setActiveTab(tab);
    setEmail(tab === 'admin' ? 'admin@tegal.go.id' : 'petugas@ecoscan.com');
    setPassword('');
        ...
        <form onSubmit={handleLogin} className="w-full space-y-5">
          <div className="relative">
            ...
            <input
              ...
              placeholder={defaultEmailByTab}
              required
            />
            <p className="mt-2 text-[10px] text-slate-400 tracking-[0.18em] uppercase">
              {emailHelpText}
            </p>
          </div>
